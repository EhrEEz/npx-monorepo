...
export function initApproachSectionAnimations(): () => void {
	const ctx = gsap.context(() => {
		...
		gsap.set('.approach__title', { opacity: 0 });
		...
		gsap.to('.approach__title', {
			...
			scrambleText: { text: '{original}', chars: 'upperAndLowerCase', revealDelay: 0.5, speed: 0.8 }
		});
		...
		mm.add('(max-width:991px)', () => {
			...
			tl.addLabel('start').fromTo(
				'.approach__subtitle',
				{ opacity: 0, yPercent: 50, duration: 0.7, ease: 'power1.out' },
		...
		approachItems.forEach((item, index) => {
			...
			tl.from(item, { yPercent: 50, opacity: 0.2, ease: 'back.out', duration: 3 });
		});
	});
